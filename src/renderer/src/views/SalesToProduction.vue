<template>
  <div class="pitopl-container">
    ...
    <div class="card">
      ...
      <button class="btn-primary" @click="generateProductionOrder" :disabled="!canGenerate">
        {{ processing ? '生成中...' : '生成生产单' }}
      </button>
    </div>
...
const addContracts = async () => {
  const files = await window.electronAPI.selectSalesFiles()
  for (const file of files) {
    if (contractFiles.value.includes(file)) continue
    
    const result: any = await window.electronAPI.validateSalesContract(file)
    if (!result.valid) {
      statusText.value = `文件无效: ${result.message}`
      continue
    }
    
    const addResult: any = await window.electronAPI.addSalesContract(file)
...
const removeContract = async (file: string) => {
  await window.electronAPI.removeSalesContract(file)
...
const refreshArticles = async () => {
  ...
  for (const file of contractFiles.value) {
    const result: any = await window.electronAPI.getSalesContractArticles(file)
    if (result.success) {
...
const selectOutput = async () => {
  const path = await window.electronAPI.selectSalesSavePath()
...
const generateProductionOrder = async () => {
  ...
  const result: any = await window.electronAPI.generateProductionOrder(outputPath.value, selected)
...
onMounted(() => {
  const saved = store.getSalesToProductionState()
...
watch([contractFiles, articles, selectedArticles, outputPath], () => {
  store.setSalesToProductionState({
